...
if ( ! function_exists( 'alx_dynamic_css' ) ) {
	...
	function alx_dynamic_css() {
		if ( ot_get_option('dynamic-styles') != 'off' ) {
			...
			// primary color
			if ( ot_get_option('color-1') != '#26abd3' ) {
				$styles .= '
...
.jp-play-bar,
.jp-volume-bar-value { background-color: '.ot_get_option('color-1').'; }
...
@media only screen and (min-width: 720px) {
	...
	#nav-header .nav li.current-post-parent > a { color: '.ot_get_option('color-1').'; }
}
...
.wp-pagenavi span.current { border-bottom-color: '.ot_get_option('color-1').'!important; }				
			...
			if ( ot_get_option('color-bubble') != '#dd5827' ) {
				$styles .= '
.post-comments { background-color: '.ot_get_option('color-bubble').'; }
.post-comments span:before { border-right-color: '.ot_get_option('color-bubble').'; border-top-color: '.ot_get_option('color-bubble').'; }				
				'."\n";
			}			
			// footer menu color
			if ( ot_get_option('color-footer-menu') != '#333333' ) {
				$styles .= '
