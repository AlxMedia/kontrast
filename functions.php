...
/*  Custom navigation
/* ------------------------------------ */
if ( ! class_exists( '\Kontrast\Nav' ) ) {
	require_once 'functions/nav.php';
}
add_action( 'wp', function() {
	$nav = new \Kontrast\Nav();
	$nav->enqueue(
		[
			'script' => 'js/nav.js',
			'inline' => false,
...
if ( ! function_exists( 'kontrast_sidebars' ) ) {
	...
	function kontrast_sidebars()	{
		...
		if ( get_theme_mod('header-ads') == 'on' ) { register_sidebar(array( 'name' => esc_html__('Header Ads','kontrast'),'id' => 'header-ads', 'description' => esc_html("Header ads area","kontrast"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3 class="group"><span>','after_title' => '</span></h3>')); }
		if ( get_theme_mod('footer-ads') == 'on' ) { register_sidebar(array( 'name' => esc_html__('Footer Ads',"kontrast"),'id' => 'footer-ads', 'description' => esc_html__("Footer ads area","kontrast"), 'before_widget' => '<div id="%1$s" class="widget %2$s">','after_widget' => '</div>','before_title' => '<h3 class="group"><span>','after_title' => '</span></h3>')); }
...
if ( ! function_exists( 'kontrast_styles' ) ) {
	...
	function kontrast_styles() {
		...
		wp_enqueue_style( 'kontrast-responsive', get_template_directory_uri().'/responsive.css' );
		wp_enqueue_style( 'kontrast-font-awesome', get_template_directory_uri().'/fonts/all.min.css' );
	}
...
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_sidebar_secondary' ) ) {

	function kontrast_sidebar_secondary() {
		// Default sidebar
		$sidebar = 'secondary';
...
/*  Social links
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_social_links' ) ) {

	function kontrast_social_links() {
...
/*  Blog title
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_blog_title' ) ) {
...
/*  Related posts
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_related_posts' ) ) {

	function kontrast_related_posts() {
		wp_reset_postdata();
		global $post;
...
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_post_images' ) ) {

	function kontrast_post_images( $args=array() ) {
		global $post;
...
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_get_featured_post_ids' ) ) {
...
/*  Body class
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_body_class' ) ) {

	function kontrast_body_class( $classes ) {
		$classes[] = kontrast_layout_class();
		if ( get_theme_mod( 'boxed','off' ) != 'on' ) { $classes[] = 'full-width'; }
		if ( get_theme_mod( 'boxed','off' ) == 'on' ) { $classes[] = 'boxed'; }
		if ( get_theme_mod( 'light-sidebar','off' ) == 'on' ) { $classes[] = 'light-sidebar'; }
...
add_filter( 'embed_oembed_html', 'kontrast_embed_html', 10, 3 );
...
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_pre_get_posts' ) ) {

	function kontrast_pre_get_posts( $query ) {
		...
		if ( $query->is_home() ) {
			...
			if ( get_theme_mod('featured-posts-count','3') != '0' ) {
				// Get featured post ids
				$featured_post_ids = kontrast_get_featured_post_ids();
				// Exclude posts
...
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_html_js_class' ) ) {
...
/* ------------------------------------ */
function kontrast_wc_wrapper_start() {
	echo '<div class="content">';
	echo '<div class="pad">';
}
function kontrast_wc_wrapper_end() {
	echo '</div>';
	echo '</div>';
}
...
/* ------------------------------------ */
if ( ! function_exists( 'kontrast_flexslider_gallery' ) ) {

	function kontrast_flexslider_gallery() {
...
/* ------------------------------------ */
function kontrast_skip_link_focus_fix() {
	?>
	<script>
...
add_action( 'wp_print_footer_scripts', 'kontrast_skip_link_focus_fix' );
